...
function renderIndustries() {
  ...
  grid.innerHTML = DATA.industries.map((ind, i) => `
    <div class="industry-card" data-aos="fade-up" data-aos-delay="${(i % 3) * 100}" tabindex="0" role="article">
      <div class="industry-bg" style="background: ${bgColors[i % bgColors.length]}" aria-hidden="true">${ind.emoji}</div>
...
function renderSolutions() {
  ...
  grid.innerHTML = DATA.solutions.map((sol, i) => `
    <div class="solution-card" data-aos="fade-up" data-aos-delay="${(i % 5) * 80}" tabindex="0" role="article">
      ...
      <div class="solution-icon" aria-hidden="true"><i class="${sol.icon}"></i></div>
      <h3>${sol.title}</h3>
    </div>
  `).join('');
}
...
(function () {
  ...
  function showFeedback(form, message, type) {
    ...
    el.style.cssText = `
      ...
      font-weight: 500;
      margin-top: 0.5rem;
      background: ${type === 'success' ? 'linear-gradient(135deg,rgba(184,164,122,0.15),rgba(184,164,122,0.08))' : 'rgba(220,53,69,0.1)'};
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (heroForm) {
      handleFormSubmit(heroForm, '✓ Thank you! Our team will contact you within 24 hours.');
    }

    if (consultForm) {
      handleFormSubmit(consultForm, '✓ Consultation scheduled! We\'ll reach out within 24 hours to confirm your session.');
    }
  });
})();
